...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-slate-800 mt-2">Edit Pesawat</h1>
</div>
    ...
    <form action="{{ route('admin.airplanes.update', $airplane) }}" method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="airline_id" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-brand-accent focus:border-brand-accent" required>
                    @foreach($airlines as $airline)
                        <option value="{{ $airline->id }}" {{ old('airline_id', $airplane->airline_id) == $airline->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="model" value="{{ old('model', $airplane->model) }}" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-brand-accent focus:border-brand-accent" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nomor Registrasi</label>
                <input type="text" name="registration_number" value="{{ old('registration_number', $airplane->registration_number) }}" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-brand-accent focus:border-brand-accent uppercase" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Kapasitas Kursi</label>
                <input type="number" name="capacity" value="{{ old('capacity', $airplane->capacity) }}" min="10" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-brand-accent focus:border-brand-accent" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-brand-accent focus:border-brand-accent">{{ old('description', $airplane->description) }}</textarea>
            </div>
        </div>
